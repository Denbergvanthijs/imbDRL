...
num_iterations = 1_000  # Total episodes
...
global_step = tf.compat.v1.train.get_or_create_global_step()  # Counter for global train episode counter
...
random_policy = random_tf_policy.RandomTFPolicy(train_env.time_step_spec(), train_env.action_spec())  # Choose random action
...
eval_summary_writer = tf.compat.v2.summary.create_file_writer(logs_dir)
...
for _ in range(num_iterations):
    ...
    if not (step := agent.train_step_counter) % log_interval:
        print(f"step={step.numpy()}; {train_loss=:.6f}")

    # https://github.com/tensorflow/agents/issues/385
    if not agent.train_step_counter % eval_interval:
        metric_utils.eager_compute(eval_metrics, eval_env, agent.policy, num_episodes=num_eval_episodes,
                                   train_step=global_step, summary_writer=eval_summary_writer, use_function=False)
