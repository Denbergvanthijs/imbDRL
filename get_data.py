...
def load_image(data_source: str):
    ...
    X_train = X_train / 255  # /= is not available when casting int to float: https://stackoverflow.com/a/48948461/10603874
    X_test = X_test / 255
...
def load_creditcard(fp_train: str = "./data/credit0.csv", fp_test: str = "./data/credit1.csv", normalization: bool = False):
    ...
    """
    X_train = read_csv(fp_train)  # Directly converted to float64
    X_test = read_csv(fp_test)

    y_train = X_train["Class"]  # 1: Fraud/Minority, 0: No fraud/Majority
    y_test = X_test["Class"]
